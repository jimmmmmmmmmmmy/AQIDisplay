...
class TestAQIVisualizationView(unittest.TestCase):
    def setUp(self):
        ...
        self.view = AQIVisualizationView.alloc().initWithFrame_andData_andTempUnit_(
            self.frame, sample_data, "°F"
        )
        self.view.setup()
    ...
    def test_get_color_for_metric(self):
        ...
        self.assertEqual(
            self.view.get_color_for_metric("pm25", 25), NSColor.systemGreenColor()
        )
        self.assertEqual(
            self.view.get_color_for_metric("temperature", 25), NSColor.orangeColor()
        )
        self.assertEqual(
            self.view.get_color_for_metric("o3", 40),
            self.view.interpolate_colors(
                NSColor.systemGreenColor(), NSColor.yellowColor(), 0.8
            ),
        )
...
class TestSearchCityWindow(unittest.TestCase):
    ...
    def test_performSearch_(self, mock_get):
        ...
        mock_get.assert_called_with(
            "https://api.waqi.info/search/?token=test_token&keyword=San Francisco"
        )
    ...
    def test_tableViewSelectionDidChange_(self):
        ...
        self.window.results = [
            {"station": {"name": "San Francisco"}, "aqi": "45", "uid": 123}
        ]
